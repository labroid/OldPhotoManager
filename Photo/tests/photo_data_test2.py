...
import logging
import datetime
import photodb.photodblib as pdl
...
def test_photo_data_instantiation():
    global photodata
    photodata = pdl.Photos(archive, repository, host, test_photos_base, create_new=True)
    assert isinstance(photodata, pdl.Photos)

def test_list_paths():
    photodata.list_paths()
    assert(True)
